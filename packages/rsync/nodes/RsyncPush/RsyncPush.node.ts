...
export class RsyncPush implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'Rsync Push',
    name: 'rsyncPush',
    icon: 'file:rsyncPush.svg',
    group: ['transform'],
    version: 1,
    subtitle: '={{$parameter["localPath"] + " -> " + $parameter["remotePath"]}}',
    description: 'Push files to a remote host using rsync over SSH',
    defaults: {
      name: 'Rsync Push',
    },
    ...
    properties: [
      {
        ...
        required: true,
        description: 'Local source path visible to the n8n container. Add a trailing slash to copy the contents of a directory.',
      },
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex += 1) {
      try {
        ...
        const destination = buildRemotePath(credentials, remotePath);
        ...
        args.push('-e', buildSshCommand(credentials), localPath, destination);

        const result = await runRsync(rsyncBinary, args, commandTimeoutSeconds * 1000);

        if (result.exitCode !== 0) {
          throw new Error(
            `rsync push failed with exit code ${result.exitCode}.${result.stderrLines.length > 0 ? ` ${result.stderrLines.join('\n')}` : ''}`,
          );
        }

        returnData.push({
          json: {
            rsyncPush: resultToJson('push', rsyncBinary, args, localPath, destination, result),
          },
          pairedItem: {
            item: itemIndex,
          },
